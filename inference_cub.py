...
def visualize_attention(img, att_maps, title='', img_save_path=None):
    ...
    ax.imshow(np.transpose(img, [1, 2, 0]), cmap='gray', alpha=0.3)
...
def visualize(x, y, y_pred, att_maps, batch_id):
    ...
    for batch_index in range(batch_size):
        ...
        img = denormalize(x[batch_index])
        visualize_attention(img.cpu().numpy(), attention_map[batch_index].cpu().numpy(), title, img_save_path)
...
def evaluate():
    ...
    for batch_id, (data, target) in enumerate(test_loader):
        ...
        y_pred, x_reconst, v_length, c_maps = capsule_net(data, target_ohe)
